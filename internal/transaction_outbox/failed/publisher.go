...
func (p *Publisher) Publish(ctx context.Context, e *models.Event) error {
	accrual := events.AccrualProcessed{
		Event: &events.AccrualProcessed_FailedEvent{
			FailedEvent: &events.AccrualFailedEvent{
	...
	if err != nil {
		return fmt.Errorf("transaction_outbox/failed/publisher: marashal failed  %w", err)
	}
	...
	if err := p.writer.WriteMessages(
		...
		return fmt.Errorf("transaction_outbox/failed/publisher: write message to toppic failed %w", err)
	}
